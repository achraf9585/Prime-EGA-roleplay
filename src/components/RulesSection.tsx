...
export default function RulesSection({ t }: { t: { serverRules: { title: string; subtitle: string; rules: Rule[] } } }) {
  ...
  return (
    <section id="rules" className="py-24 px-4 relative overflow-hidden">
      {/* Background Elements */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#0a0514] via-[#0a0514] to-[#0a0514]" />
      ...
      <div className="container mx-auto relative z-10">
        <div className="text-center mb-20">
          ...
          >
            <h2 className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#c084fc] via-[#8b5cf6] to-[#3b82f6] mb-6">
              {t.serverRules.title}
            </h2>
          ...
          {t.serverRules.rules.map((rule: Rule, index: number) => {
            ...
            return (
              <motion.div key={index} variants={item}>
                <Card className="h-full bg-[#0a0514]/85 backdrop-blur-sm border-[#1a103c] hover:border-[#8b5cf6]/50 transition-all duration-300 group overflow-hidden relative">
                  <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                    <Icon className="w-24 h-24 text-[#8b5cf6]/60" />
                  </div>
                  
                  <CardHeader className="relative pb-2">
                    <div className="flex items-center gap-4 mb-2">
                      <div className="w-10 h-10 rounded-lg bg-[#8b5cf6]/10 flex items-center justify-center border border-[#8b5cf6]/20 group-hover:bg-[#8b5cf6]/20 transition-colors">
                        <span className="text-[#8b5cf6] font-bold font-mono">
                          {String(index + 1).padStart(2, '0')}
                        </span>
                      </div>
                      <CardTitle className="text-xl text-slate-100 group-hover:text-[#8b5cf6] transition-colors">
                        {rule.title}
                      </CardTitle>
                    </div>
                  </CardHeader>
